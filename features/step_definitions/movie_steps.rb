...
When /^I click button (.*)$/ do |pressed|
  click_button(pressed)
end

When /^I click link (.*)$/ do |pressed|
  click_link(pressed)
end
...
Then /I should (not)?\s?see movies with the following ratings: (.*)?/ do |no, ratings_list|
  #p page.text
  movies = Movie.where "rating in (?)", ratings_list.split(/,\s?/)
  #p movies
  movies.each do |movie|
    if page.respond_to? :should
      ...
      no == "not" ? assert(page.has_no_content?(movie.title)) : assert(page.has_content?(movie.title))
    end
  end
end
